...
def reverse_every_element_in_array(array)
  array.reverse_each{|element| element.reverse!}
end
...
def get_first_half_of_string(string)
  middle = string.length.even? ? string.length / 2 -1 : string.length / 2
  string[0..middle]
end
...
def number_of_elements_that_are_palindromes(array)
  number = 0
  array.each{|element| number+=1 if element == element.reverse}
  number
end

# return the shortest word in an array
def shortest_word_in_array(array)
  array.group_by(&:size).min.last.first
end

# return the longest word in an array
def longest_word_in_array(array)
  array.group_by(&:size).max.last.first
end
...
def get_elements_until_greater_than_five(array)
  index = array.find_index { |element| element > 5 }
  array[0..index - 1]
end
...
def titleize_a_string(string)
  ignore = ["a","and","the"]
    string.split(' ').each_with_index{|word,index| word.capitalize! if index ==0 || !ignore.include?(word)}.join(' ')
end
...
def is_a_2014_bank_holiday?(date)
  bank_holidays_list = [ [1,1],[4,18],[4,21],[5,5],[5,26],[8,25], [12,25], [12,26]]
  bank_holidays_list.find { |month, day| date == Time.new(2014, month, day) }
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  ...
  until next_birthday.friday?
    if Date.new(next_birthday.year).leap?
      next_birthday += 31622400
    else
      next_birthday += 31536000
    end
  end
  next_birthday.year
